...
async def test_stage_and_commit_replay_file_writes_replay_data(tmp_path) -> None:
    score = _score()
    score.client_checksum = "abc123"
    replay_data = b"x" * score_submission.MIN_REPLAY_SIZE

    pending_path = await score_submission.stage_replay_file(
        replay_data=replay_data,
        replays_path=tmp_path,
        checksum=score.client_checksum,
    )

    # staged, but not yet under its final name -- the score row does not
    # exist at this point, so the score id is not yet known.
    assert pending_path is not None
    assert pending_path.read_bytes() == replay_data
    assert not (tmp_path / "42.osr").exists()

    score_submission.commit_replay_file(
        score,
...
async def test_stage_replay_file_ignores_missing_replay_data(tmp_path) -> None:
    pending_path = await score_submission.stage_replay_file(
        replay_data=None,
        replays_path=tmp_path,
        checksum="abc123",
...
async def test_discard_staged_replay_removes_pending_file(tmp_path) -> None:
    """A score that fails to persist must not leave its replay behind."""
    pending_path = await score_submission.stage_replay_file(
        replay_data=b"x" * score_submission.MIN_REPLAY_SIZE,
        replays_path=tmp_path,
        checksum="abc123",
    ...
    score_submission.discard_staged_replay(pending_path)

    assert not pending_path.exists()
    assert list(tmp_path.iterdir()) == []


def test_discard_staged_replay_tolerates_missing_file(tmp_path) -> None:
    # idempotent: discarding twice (or after a crash) must not raise.
    score_submission.discard_staged_replay(tmp_path / "gone.osr.pending")


def test_commit_replay_file_ignores_missing_pending_path() -> None:
    score_submission.commit_replay_file(
        _score(),
        pending_path=None,
        replays_path=Path("/nonexistent"),
    )
...
def _score_submission_service(
    *,
    ...
    bancho_authentication: object | None = None,
...
async def test_persist_submitted_score_forwards_pp_skill_components() -> None:
    # Track 3.3: the aim/speed/flashlight breakdown computed during performance
    # calculation must reach the scores row, not be dropped as it was before.
    score = _score()
    score.pp_aim = 6.5
    score.pp_speed = 3.25
    score.pp_flashlight = 0.75
    scores = _FakeScoresRepository()

    await score_submission.persist_submitted_score(score, scores)

    created = scores.created_scores[0]
    assert created["pp_aim"] == 6.5
    assert created["pp_speed"] == 3.25
    assert created["pp_flashlight"] == 0.75


async def test_persist_submitted_score_forwards_null_skill_components_untouched() -> (
    None
):
    # a score whose beatmap file was unavailable never runs performance
    # calculation, so the components stay None; that None must be persisted
    # verbatim (the column is nullable) rather than coerced to 0.0.
    score = _score()
    assert score.pp_aim is None
    assert score.pp_speed is None
    assert score.pp_flashlight is None
    scores = _FakeScoresRepository()

    await score_submission.persist_submitted_score(score, scores)

    created = scores.created_scores[0]
    assert created["pp_aim"] is None
    assert created["pp_speed"] is None
    assert created["pp_flashlight"] is None
...
async def test_submit_score_orchestrates_submission_side_effects(
    ...
    service = _score_submission_service(
        ...
        bancho_authentication=player_authenticator,
...
async def test_submit_score_rejects_duplicate_inside_submission_lock(tmp_path) -> None:
    ...
    service = _score_submission_service(
        ...
        bancho_authentication=_FakePlayerAuthenticator(player),
        score_submission_locks=locks,
        database=_FakeDatabaseTransactions(),
        scores=scores,
        stats=_FakeStatsRepository(),
