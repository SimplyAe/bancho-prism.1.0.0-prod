...
class ScoresTable(Base):
    ...
    # Prism (Track 3.3): the akatsuki_pp_py skill-component breakdown of `pp`.
    # Nullable and additive -- older rows and modes the calculator does not break
    # down leave them NULL, and nothing on the leaderboard hot path reads them,
    # so they carry no index.
    pp_aim = Column("pp_aim", FLOAT(precision=7, scale=3), nullable=True)
    pp_speed = Column("pp_speed", FLOAT(precision=7, scale=3), nullable=True)
    pp_flashlight = Column("pp_flashlight", FLOAT(precision=7, scale=3), nullable=True)
...
READ_PARAMS = (
    ...
    ScoresTable.pp_aim,
    ScoresTable.pp_speed,
    ScoresTable.pp_flashlight,
)
...
class Score:
    ...
    perfect: int
    online_checksum: str
    # Prism (Track 3.3) skill-component breakdown; nullable, defaulted so the
    # many keyword constructions of this DTO need not be touched.
...
class ScoresRepository:
    ...
    def _deserialize_score(self, row: MySQLRow) -> Score:
        return Score(
            ...
            pp_aim=row["pp_aim"],
            pp_speed=row["pp_speed"],
            pp_flashlight=row["pp_flashlight"],
        )
    ...
    async def fetch_user_id(self, score_id: int) -> int | None:
        """The owning user id for a score, or None if the score is gone.

        A single-column lookup used off the hot path (the anticheat review queue
        denormalises the owner onto a flag), so it deliberately avoids
        materialising a whole ``Score`` for one field.
        """
        select_stmt = select(ScoresTable.userid).where(ScoresTable.id == score_id)
        row = await self._database.fetch_one(select_stmt)
        return int(row["userid"]) if row is not None else None
