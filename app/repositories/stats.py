...
@dataclass(frozen=True, slots=True)
class RankedPlayerPP:
    """A single player's pp for one mode, for leaderboard rebuilds."""
...
class StatsRepository:
    ...
    async def fetch_ranked_pp_for_mode(
        ...
        *,
        ...
        """Fetch (player_id, country, pp) for ranked players in a mode.

        Used to rebuild the redis leaderboards from mysql, which is the
        source of truth. Results are keyset-paginated on `player_id` --
        rather than LIMIT/OFFSET -- so a rebuild over a large table does
        not degrade quadratically, and cannot skip or duplicate rows if
        stats change mid-walk.

        The ranked-player predicate is deliberately identical to
        `fetch_leaderboard_stats_rows`, so the rebuilt sorted sets agree
        with the SQL-backed leaderboard about who is ranked.
        """
        select_stmt = (
            select(
                UsersTable.id.label("player_id"),
