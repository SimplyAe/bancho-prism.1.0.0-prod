...
class UsersRepository:
    ...
    async def fetch_by_api_key(self, api_key: str) -> User | None:
        """Fetch the user owning an api key, or None if unrecognized.

        Authoritative against the database on every call -- deliberately
        not served from an in-memory snapshot. A key issued (or revoked)
        after startup takes effect immediately, and there is no unbounded
        startup load of every key into RAM. The `users_api_key_uindex`
        unique index keeps this a single indexed lookup.
        """
        select_stmt = select(*READ_PARAMS, UsersTable.api_key).where(
            UsersTable.api_key == api_key,
        )
        user = await self._database.fetch_one(select_stmt)
        return self._deserialize_user(user) if user is not None else None
