...
class Score:
    ...
    def __init__(self) -> None:
        ...
        # akatsuki_pp_py breaks `pp` down into these skill components on every
        # submission; they were previously computed and discarded. Persisting
        # them (Track 3.3) gives the anticheat behavioural baselines and
        # per-skill analysis something to read. None until `calculate_performance`
        # runs (the same condition as `pp`), and None for the components the
        # calculator does not break out for a given mode.
        self.pp_aim: float | None = None
        self.pp_speed: float | None = None
        self.pp_flashlight: float | None = None
    ...
    def calculate_performance(self, beatmap_id: int) -> tuple[float, float]:
        ...
        # carry the skill-component breakdown onto the score so the submission
        # path can persist it alongside the total pp (Track 3.3).
        self.pp_aim = performance.pp_aim
        self.pp_speed = performance.pp_speed
        self.pp_flashlight = performance.pp_flashlight

        return performance.pp, result[0].difficulty.stars
