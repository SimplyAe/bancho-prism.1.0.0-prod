...
async def bancho_handler(
    ...
    *,
    ...
    ip = app.state.services.ip_resolver.get_ip(
        request.headers,
        peer_host=request.client.host if request.client else None,
    )
    if ip is None:
        # we cannot geolocate, rate-limit, or hardware-match a client we
        # cannot identify; refuse rather than logging in with a bogus IP.
        log("Refusing bancho request with unresolvable client IP.", Ansi.LRED)
