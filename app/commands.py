...
async def apikey(ctx: Context) -> str | None:
    ...
    # generate new token. api-key auth reads the database directly
    # (see UsersRepository.fetch_by_api_key), so the new key -- and the
    # revocation of the old one -- takes effect immediately, with no
    # in-process cache to keep in sync.
