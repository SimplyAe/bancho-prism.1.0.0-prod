...
.PHONY: build run run-bg run-cfd run-cfd-bg run-caddy logs shell test utest lint type-check install install-dev uninstall bump backup restore-drill
...
# Nightly backup: logical dump (PITR base) + binlog archive + replay snapshot.
# Run from cron/systemd. See docs/runbooks for how the artefacts are used.
backup:
	scripts/backup.sh

# Restore drill: load the latest backup into a throwaway instance and verify it
# (tables present, indexes intact, measured RTO). Run weekly. `--with-binlogs`
# also exercises the full point-in-time roll-forward.
restore-drill:
